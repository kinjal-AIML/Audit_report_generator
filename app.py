...
def extract_data_route():
    try:
        ...
        print(f"Extracting data for file: {audit_file.filename}, start_date: {start_date}")

        # Save audit file
        audit_path = os.path.join(UPLOAD_DIR, audit_file.filename)
        audit_file.save(audit_path)
        
        # Handle supporting PDFs
        ...
        for f in pdf_files:
            if f.filename:
                ...
                # Dynamic mapping based on filename or content
                clean_name = f.filename.lower()
                ...
                elif "npa" in clean_name:
                    file_mappings["NPA Accounts"] = f.filename
                elif "overdue" in clean_name:
                    file_mappings["Loan Overdue"] = f.filename
                elif "recovery" in clean_name:
                    file_mappings["Recovery Accounts"] = f.filename
                else:
                    # Use filename (without extension) as sheet name if no match
                    name_without_ext = os.path.splitext(f.filename)[0]
                    file_mappings[name_without_ext] = f.filename

        # Extract data from Audit Template (DOCX)
        if "CNSB" in audit_file.filename.upper():
            ...
            template_name = "UPDATED_CNSB_TEMPLATE.docx" # Default
            data = {}

        # Auto-generate dates
        if not start_date:
            start_date = "2026-02-01" # Fallback
            
        date_context = build_template_context("Branch X", "Ahmedabad", start_date)
        data.update(date_context)

        # Initialize placeholders from query
        placeholders = {
        ...
        # Build a DOCX→Excel output (single sheet) for direct download in Step 2
        try:
        ...
        # Extract data from PDFs only for preview tabs
        pdf_results = process_all_pdfs(UPLOAD_DIR, file_mappings)
        
        # Generate intelligent summaries for review (Step 2)
        npa_df = pdf_results.get("npa", pd.DataFrame())
        if not npa_df.empty:
        ...
        # Convert DataFrames to JSON-serializable format for review
        import numpy as np
